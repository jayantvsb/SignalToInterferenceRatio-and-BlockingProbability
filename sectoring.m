...
%REFERENCE BASE STATION COORDINATES IN ij COORDINATE SYSTEM
vertices_x = [0 R_c/2 R_c R_c/2 0];                       %i-axis coordinates
vertices_y = [0 cosd(30)*R_c 0 -cosd(30)*R_c 0];                        %j-axis coordinates


%Below is the interfering cells coordinates in ij coordinate system with sectoring in reuse-1 condition
interference_x = 2*cosd(30)*R_c*[-1 -1 -1 -1 -2 -2 -2];              %i-axis coordinates
interference_y = 2*cosd(30)*R_c*[0 1 2 -1 0 1 2];                    %j-axis coordinates


d = zeros(1,7);          %distance from rand0m location to the interfering cells and made them to zeros
...
while (n ~= total_points)
 ...
 %Below we convert ij coordinate to xy coordinate                                                  
  ...
  Y = y + sind(30)*x;                                 %here we get the x coordinate in XY coordinate system
